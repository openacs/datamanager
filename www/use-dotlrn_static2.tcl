...
switch $action {
    "copy" {
            foreach community $dest_community_id {
                foreach object $object_id {
                    ...
                    if { [catch {callback datamanager::copy_static -object_id $object -selected_community $community -sufix $sufix} errmsg] } {
    ...
    "export" {
            foreach community $dest_community_id {
                foreach object $object_id {
                    dotlrn::require_user_admin_community  -community_id $community
                    if { [catch {callback datamanager::export_static -object_id $object -selected_community $community -sufix $sufix} errmsg] } {
                      ad_returnredirect [append tmp $error_url "?error=" $errmsg]
                      ad_script_abort             
                    } else {
                      set items_ok [expr $items_ok + 1]
